...
export function ChartAreaInteractive() {
  ...
  // Shift static chart data so the last element represents "today",
  // making it align with the current date context.
  const adjustedChartData = React.useMemo(() => {
    const today = new Date();
    return chartData.map((item, idx) => {
      const diffDays = chartData.length - 1 - idx;
      const itemDate = new Date(today);
      itemDate.setDate(today.getDate() - diffDays);
      ...
      return {
        date: `${year}-${month}-${day}`,
        kotor: item.desktop * 100000,
        bersih: item.mobile * 100000,
      };
    });
  }, []);
  ...
  const timeRange = React.useMemo(() => {
    ...
    const isEndingToday = date.to.toDateString() === todayStr;
  ...
  const filteredData = React.useMemo(() => {
    if (!date?.from) return adjustedChartData;
    ...
    return adjustedChartData.filter((item) => {
      const itemDate = new Date(item.date);
      return itemDate >= from && itemDate <= to;
    });
  }, [date, adjustedChartData]);
      ...
      <CardHeader>
        ...
        <CardAction>
          <ToggleGroup
            ...
            onValueChange={(value) => {
              const val = value[0];
              if (val) handleTimeRangeChange(val);
            }}
            variant="outline"
      ...
      <CardContent className="px-2 pt-4 sm:px-6 sm:pt-6">
        ...
        >
          <AreaChart data={filteredData}>
            <defs>
              <linearGradient id="fillKotor" x1="0" y1="0" x2="0" y2="1">
                <stop
                  offset="5%"
                  stopColor="var(--color-kotor)"
                  stopOpacity={0.4}
                />
                <stop
                  offset="95%"
              ...
              <linearGradient id="fillBersih" x1="0" y1="0" x2="0" y2="1">
                <stop
                  offset="5%"
                  stopColor="var(--color-bersih)"
                  stopOpacity={0.4}
                />
                <stop
                  offset="95%"
                  ...
                  formatter={(value, name, item) => (
                    <>
                      ...
                      <div className="flex flex-1 justify-between items-center leading-none">
                        ...
                        <span className="font-mono font-semibold text-foreground tabular-nums ml-4 text-xs">
