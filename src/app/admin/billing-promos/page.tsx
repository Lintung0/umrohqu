...
import { Tag, Plus, Edit, Trash2, Percent, DollarSign, Loader2, X } from "lucide-react"
...
export default function AdminBillingPromosPage() {
  ...
  async function fetchPromos() {
    ...
    const { data } = await supabase
      .from("promotions")
      .select("id, title, description, code, discount_type, discount_value, min_booking, max_usage, valid_until, is_active, tenant_id")
      .is("deleted_at", null)
            ...
            promos.map((promo) => (
              <div key={promo.id} className="flex items-center gap-4 p-5 hover:bg-gray-50/50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">{promo.description || "Diskon: "}{promo.discount_type === "discount_percent" ? `${promo.discount_value}%` : formatRupiah(promo.discount_value)} · Min: {promo.min_booking ? formatRupiah(promo.min_booking) : "-"} · Hingga: {promo.valid_until ? new Date(promo.valid_until).toLocaleDateString("id-ID") : "-"}</p>
                </div>
